...
eventPermissionSchema.index({ "submittedBy.email": 1 });
eventPermissionSchema.index({ "recipientsFlow.email": 1 });
eventPermissionSchema.index({ status: 1 });
eventPermissionSchema.index({ currentStage: 1 });

// Virtual to get current recipient
eventPermissionSchema.virtual("currentRecipient").get(function () {
...
eventPermissionSchema.methods.canUserSign = function (userEmail) {
...
eventPermissionSchema.methods.advanceStage = function () {
    this.currentStage += 1;
...
eventPermissionSchema.methods.addSignature = function (signatureData) {
    this.signatures.push(signatureData);
    this.advanceStage();
};

// Set virtuals to be included in JSON
eventPermissionSchema.set("toJSON", { virtuals: true });
eventPermissionSchema.set("toObject", { virtuals: true });
