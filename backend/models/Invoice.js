...
invoiceSchema.index({ "submittedBy.email": 1 });
invoiceSchema.index({ "recipientsFlow.email": 1 });
invoiceSchema.index({ status: 1 });
invoiceSchema.index({ currentStage: 1 });
invoiceSchema.index({ eventPermissionId: 1 });

// Virtual to get current recipient
invoiceSchema.virtual("currentRecipient").get(function () {
...
invoiceSchema.methods.canUserSign = function (userEmail) {
...
invoiceSchema.methods.advanceStage = function () {
    this.currentStage += 1;
...
invoiceSchema.methods.addSignature = function (signatureData) {
