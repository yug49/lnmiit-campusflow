...
def lambda_handler(event, context):
    """
    Get all MOUs with optional filtering by status
    Query params: ?status=PENDING_FACULTY or ?status=APPROVED
    """
    ...
    try:
        ...
        return {
            ...
            'body': json.dumps({'error': f'Failed to fetch MOUs: {str(e)}'}),
            'isBase64Encoded': False
        }
