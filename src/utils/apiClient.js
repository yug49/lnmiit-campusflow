...
axiosInstance.interceptors.request.use(
    (config) => {
        // Use Privy token for authentication
...
const api = {
    // Auth endpoints
    auth: {
        // Get user by email (for role lookup)
        getUserByEmail: async (email, privyToken) => {
            try {
    ...
    users: {
        ...
        createUser: async (userData) => {
            try {
                return await axiosInstance.post("/users/create", userData);
            } catch (error) {
    ...
    noDues: {
        ...
        rejectRequest: async (id, reason) => {
            try {
                return await axiosInstance.put(`/nodues/reject/${id}`, {
                    reason,
                });
            } catch (error) {
    ...
    mous: {
        ...
        rejectMOU: async (id, reason) => {
            try {
                return await axiosInstance.put(`/mous/reject/${id}`, {
                    reason,
                });
            } catch (error) {
    ...
    events: {
        ...
        rejectEvent: async (id, reason) => {
            try {
                return await axiosInstance.put(`/events/reject/${id}`, {
                    reason,
                });
            } catch (error) {
    ...
    voting: {
        ...
        updateCandidaturePortalStatus: async (isOpen) => {
            try {
                return await axiosInstance.put(
                    "/voting/candidature-portal-status",
                    {
                        isOpen,
                    }
                );
