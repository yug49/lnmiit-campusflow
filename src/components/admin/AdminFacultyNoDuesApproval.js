...
const AdminFacultyNoDuesApproval = () => {
    ...
    const fetchAllNoDues = async () => {
        try {
            setLoading(true);
            const response = await api.facultyNoDues.getAllNoDues();
    ...
    const handleApprove = async (noDues) => {
        ...
        try {
            ...
            await api.facultyNoDues.signNoDues(noDues._id, signatureData);
    ...
    const handleReject = async () => {
        ...
        try {
            ...
            await api.facultyNoDues.rejectNoDues(
                selectedNoDues._id,
                rejectionReason
            );
            ...
            {filteredNoDues.length === 0 ? (
                ...
                >
                    <Typography variant="h6" sx={{ color: "#fff" }}>
                        No faculty no dues requests found
                    </Typography>
                </Paper>
            ) : (
                <TableContainer
                            ...
                            {filteredNoDues.map((noDues) => (
                                <TableRow key={noDues._id}>
                                    <TableCell sx={{ color: "#fff" }}>
                                        {noDues.facultyInfo?.name || "N/A"}
                                    </TableCell>
                                    <TableCell sx={{ color: "#fff" }}>
                                        {noDues.facultyInfo?.employeeId ||
                                            "N/A"}
                                    </TableCell>
                                    <TableCell sx={{ color: "#fff" }}>
                                        {noDues.facultyInfo?.department ||
                                            "N/A"}
                                    </TableCell>
                                    <TableCell>
                                        <Chip
...
export default AdminFacultyNoDuesApproval;
