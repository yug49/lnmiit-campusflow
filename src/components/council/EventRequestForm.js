...
const EventRequestForm = () => {
    ...
    const signDocument = async (documentHash) => {
        try {
            ...
            const message = `Event Permission Document Signature\n\nDocument Hash: ${documentHash}\n\nTitle: ${formData.title}\n\nBy signing this message, I approve and submit this event permission request.`;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!formData.title.trim()) {
                setError("Please enter an event permission title");
                setLoading(false);
                return;
            }
            ...
            setSuccess("Uploading event permission...");
            await api.eventPermission.submitEventPermission(uploadData);

            setSuccess(
                "Event permission submitted successfully! Redirecting..."
            );
            ...
            // Redirect to event status page after 2 seconds
            setTimeout(() => {
                navigate("/council/event-status");
            }, 2000);
        } catch (err) {
            console.error("Submit error:", err);
            setError(err.message || "Failed to submit event permission");
        } finally {
            setLoading(false);
        }
    };
        ...
        <Box sx={{ p: 3 }}>
            ...
            <Paper sx={{ p: 3 }}>
                ...
                <Typography
                    ...
                    sx={{ mb: 3 }}
                >
                    Upload and submit an event permission request for approval.
                    The document will be digitally signed using your Privy
                    embedded wallet.
                </Typography>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <TextField
                        fullWidth
                        label="Event Permission Title"
                        name="title"
                        value={formData.title}
                        onChange={handleInputChange}
                        required
                        sx={{ mb: 3 }}
                        placeholder="Enter a descriptive title for the event permission"
                        disabled={loading}
                    />
                    ...
                    <Box sx={{ mb: 3 }}>
                        ...
                        >
                            ...
                            sign the event permission. Each person must sign
